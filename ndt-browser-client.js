...
function NDTjs(server, port, path, callbacks) {
  ...
  this.make_login_msg = function(desired_tests) {
     ...
     */
    var i = 0;
    var msg = 'XXX { "msg": "v3.5.5", "tests": "' + (desired_tests | 16) +
        '" }';
    var data = new Uint8Array(msg.length);
    
    data[0] = _this.NDT_MESSAGES.indexOf('MSG_EXTENDED_LOGIN');
  ...
  this.parse_ndt_msg = function(buf) {
    ...
    var msg =  String.fromCharCode.apply(null,
      new Uint8Array(buf.slice(3)));
    resp.push(msg);
    return resp;
  ...
  this.ndt_meta_test = function(sock) {
    ...
    return function (type, body) {
      ...
      if (state === "WAIT_FOR_TEST_FINALIZE" &&
         type === _this.NDT_MESSAGES.indexOf('TEST_FINALIZE')) {
  ...
  this.ndt_s2c_test = function(sock) {
    ...
    return function (type, body) {
      ...
      _this.log_msg("CALLED S2C with " + type + " (" +
        _this.NDT_MESSAGES[type] + ") in state " + state);
      ...
      if (state === "WAIT_FOR_TEST_PREPARE" &&
        ...
        test_connection = new WebSocket("ws://" + _this.server + ":" +
          server_port + _this.server_path, 's2c');
      ...
      if (state === "WAIT_FOR_FIRST_TEST_MSG" &&
          type === _this.NDT_MESSAGES.indexOf('TEST_MSG')) {
        ...
        _this.s2c_rate = (8 * received_bytes / 1000 /
          (test_end - test_start));
        _this.log_msg("S2C rate calculated by client: " +
          _this.s2c_rate);
        _this.log_msg("S2C rate calculated by server: " +
          body.ThroughputValue);
        sock.send(_this.make_ndt_msg(_this.NDT_MESSAGES.indexOf('TEST_MSG'),
      ...
      if (state === "WAIT_FOR_TEST_MSG_OR_TEST_FINISH" &&
        ...
        for ( var web100var in _this.web100vars ) {
          ...
          if ( body.msg.match(re) ) {
            ...
            _this.log_msg('Set ' + web100var + ' to ' +
              _this.web100vars[web100var]);
          }
        }
        return false;
      }
      ...
      _this.log_msg("S2C: State = " + state + " type = " + type + "(" +
        NDT_MESSAGES[type] + ") message = ", body);
    };
  }
  ...
  this.ndt_c2s_test = function() {
    ...
    return function (type, body) {
      _this.log_msg("CALLED C2S with " + type + " (" +
        _this.NDT_MESSAGES[type] + ") " + body.msg + " in state " +
        state);
      if (state === "WAIT_FOR_TEST_PREPARE" &&
          type === _this.NDT_MESSAGES.indexOf('TEST_PREPARE')) {
        ...
        server_port = Number(body.msg);
        test_connection = new WebSocket("ws://" + _this.server + ":" +
          server_port + _this.server_path, 'c2s');
        test_connection.binaryType = 'arraybuffer';
        state = "WAIT_FOR_TEST_START";
        return false;
      }
      ...
      if (state === "WAIT_FOR_TEST_MSG" &&
        ...
        _this.log_msg("C2S rate calculated by server: " +
          _this.c2s_rate);
        state = "WAIT_FOR_TEST_FINALIZE";
        return false;
      }
      ...
      _this.log_msg("C2S: State = " + state + " type = " + type + "(" +
        msg_name[type] + ") message = ", body);
    };
  }
  ...
  this.ndt_coordinator = function() {
    ...
    sock = new WebSocket("ws://" + _this.server + ":" + _this.server_port +
      _this.server_path, 'ndt');
    ...
    sock.onmessage = function(e) {
      ...
      _this.log_msg("type = " + type + " (" + _this.NDT_MESSAGES[type] +
        ") body = '" + body.msg + "'");
      ...
      if (state === "LOGIN_SENT") {
        ...
        if (type === _this.NDT_MESSAGES.indexOf('SRV_QUEUE')) {
          if (body.msg === "9990") {    // special keepalive message
            sock.send(_this.make_ndt_msg(_this.NDT_MESSAGES.indexOf('MSG_WAITING'), ""));
          } else if (body.msg === "9977") {   // Test failed
            throw _this.TestFailureException("Server terminated test with " +
              "SRV_QUEUE 9977");
          }
          _this.log_msg("Got SRV_QUEUE. Ignoring and waiting for MSG_LOGIN");
        } else if (type === _this.NDT_MESSAGES.indexOf('MSG_LOGIN')) {
          if (body.msg[0] !== "v") { _this.log_msg("Bad msg " + body.msg); }
          state = "WAIT_FOR_TEST_IDS";
        } else {
           error_message = "Expected type 1 (SRV_QUEUE) or 2 (MSG_LOGIN) but got " + type + " (" + _this.NDT_MESSAGES[type] + ")";
          throw _this.TestFailureException(error_message);
        }
      } else if (state === "WAIT_FOR_TEST_IDS" &&
          type === _this.NDT_MESSAGES.indexOf('MSG_LOGIN')) {
        ...
        state = "WAIT_FOR_MSG_RESULTS";
          ...
          type === _this.NDT_MESSAGES.indexOf('MSG_RESULTS')) {
        _this.log_msg(body);
      } else if (state === "WAIT_FOR_MSG_RESULTS" &&
          type === _this.NDT_MESSAGES.indexOf('MSG_LOGOUT')) {
        sock.close();
        _this.callbacks['onchange']('finished_all');
        _this.callbacks['onfinish']();
        _this.log_msg("TESTS FINISHED SUCCESSFULLY!");
      } else {
        error_message = "Didn't know what to do with message type " +
          type + " in state " + state;
        throw _this.TestFailureException(error_message);
      }
    }
  ...
  this.log_msg= function(log_message) {
